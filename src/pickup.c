...
menu_item **pick_list;	/* list of objects and counts to pick up */
{
    ...
    for (n = 0, curr = olist; curr; curr = FOLLOW(curr, follow)) {
        ...
        if (!pickit)
            pickit = (iflags.pickup_thrown && curr->was_thrown);
 ...
 *	FEEL_COCKATRICE   - Engage cockatrice checks and react appropriately.
 *	SIGNAL_ESCAPE     - Return -2 if menu is escaped.
 */
int
...
boolean telekinesis;
{
    ...
    *cnt_p = (obj->otyp == BOULDER && maybe_polyd(throws_rocks(youmonst.data),
	                Race_if(PM_GIANT)))
	? 1
...
dotip()
{
	...
	for (cobj = level.objects[cc.x][cc.y]; cobj; cobj = nobj) {
		...
		spill_container((struct monst *)0, cobj, FALSE, !Blind);
		return 1;
	    }
	}
    }
	...
	while (cobj->spe) {
        ...
        spill_container(&youmonst, cobj, FALSE, !Blind);
    return 1;
}
