...
#define on_altar()	(IS_ALTAR(levl[u.ux][u.uy].typ) && !u.uswallow)
...
aligntyp resp_god;
{
	if (u.uswallow) {
		...
		if (Blind)
		    ...
		    (void) ureflects("%s reflects from %s %s.", "It");
...
#define MAXVALUE 24 /* Highest corpse value (besides Wiz) */
	...
	#ifdef LIVELOG
  	        livelog_conduct("rejected atheism by offering %s on an altar of %s", corpse_xname(otmp, (const char *)0, CXN_ARTICLE), a_gname())
	#endif
                ;
