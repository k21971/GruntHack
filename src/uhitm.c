...
do_rust:
	    ...
	    case AD_DRIN:
	        if ((youmonst.data == &mons[PM_ZOMBIE] || youmonst.data == &mons[PM_DRAGON_ZOMBIE]) && rn2(5))
		{
zombie:
                    if (!resists_sick(mdef))
		    {
		...
		exercise(A_WIS, TRUE);
	        if (youmonst.data == &mons[PM_ZOMBIE] || youmonst.data == &mons[PM_DRAGON_ZOMBIE]) goto zombie;
		break;
	    case AD_STCK:
		    ...
		    mdef->data->mlet == S_JABBERWOCK))
		{
		    ...
		    if (mdef->data == &mons[PM_ZOMBIE] || mdef->data == &mons[PM_DRAGON_ZOMBIE])
...
/*	These only affect you if they still live */
	...
	if(malive && !mon->mcan && rn2(3)) {
		...
		if(ptr == &mons[PM_FLOATING_EYE]) {
		    ...
		    if(mon->mcansee) {
			if (ureflects("%s gaze is reflected by %s %s.",
                                      s_suffix(Monnam(mon))));
			else if (Free_action)
